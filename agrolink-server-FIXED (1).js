...
// Top 3 Engagement Users Badge - En yüksek etkileşime sahip kullanıcılar
app.get('/api/badges/top-engagement', authenticateToken, async (req, res) => {
    try {
        // Her kullanıcının toplam beğeni ve paylaşım sayısını hesapla
        const topUsers = await db.all(`
            SELECT 
                ...
                (COUNT(DISTINCT l.id) + COUNT(DISTINCT s.id)) as totalEngagement
            FROM users u
            ...
            WHERE u.isActive = 1
            GROUP BY u.id
            HAVING totalLikes > 0 AND totalShares > 0
        ...
        res.json({ 
            ...
            topUsers: topUsers.map((user, index) => ({
                ...
                badgeLevel: index === 0 ? 'gold' : index === 1 ? 'silver' : 'bronze'
            }))
        });
    } catch (error) {
...
app.get('/api/badges/user/:userId', authenticateToken, async (req, res) => {
    try {
        ...
        // Check agricultural engineer badge
        const badges = [];
        ...
        if (engagementData && engagementData.totalLikes > 0 && engagementData.totalShares > 0) {
            ...
            if (userRank !== -1) {
                ...
                badges.push({
                    ...
                    rank: userRank + 1,
                    name: badgeName,
                    icon: userRank === 0 ? '🥇' : userRank === 1 ? '🥈' : '🥉',
                    color: userRank === 0 ? '#FFD700' : userRank === 1 ? '#C0C0C0' : '#CD7F32'
                });
            }
        }
