...
// POST OLUŞTURMA - Multer wrapper ile
app.post('/api/posts', authenticateToken, checkRestriction, (req, res, next) => {
    const uploadMiddleware = upload.array('media', UPLOAD_CONFIG.maxFilesPerUpload);
    ...
    uploadMiddleware(req, res, (err) => {
        if (err instanceof multer.MulterError) {
            ...
            if (err.code === 'LIMIT_FILE_SIZE') {
                return res.status(413).json({
                    success: false,
                    error: `Dosya boyutu çok büyük. Maksimum ${UPLOAD_CONFIG.maxFileSize / (1024 * 1024)}MB olmalıdır.`,
                    code: 'FILE_TOO_LARGE'
                });
            }
            ...
            if (err.code === 'LIMIT_FILE_COUNT') {
                return res.status(413).json({
                    success: false,
                    error: `Çok fazla dosya. Maksimum ${UPLOAD_CONFIG.maxFilesPerUpload} dosya yükleyebilirsiniz.`,
                    code: 'TOO_MANY_FILES'
                });
            }
