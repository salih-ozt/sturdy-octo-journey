...
app.post('/api/posts', authenticateToken, checkRestriction, upload.array('media', UPLOAD_CONFIG.maxFilesPerUpload), handleMulterError, async (req, res) => {
    ...
    console.log(`📝 POST isteği başladı - User: ${req.user?.id || 'unknown'}, Files: ${req.files ? req.files.length : 0}`);
    
    try {
        // 1. Temel kontroller
        if (!isDbReady) {
            console.error('❌ DB hazır değil');
            return res.status(503).json({ error: 'Veritabanı hazırlanıyor, lütfen birkaç saniye bekleyin', code: 'DB_NOT_READY' });
        }

        if (!req.user || !req.user.id) {
            console.error('❌ Oturum geçersiz');
            return res.status(401).json({ error: 'Oturum geçersiz, lütfen tekrar giriş yapın', code: 'INVALID_SESSION' });
        }
        ...
        console.log(`📋 İçerik tipi: ${isAnketMode ? 'Anket' : 'Normal'}, Dosya: ${req.files ? req.files.length : 0}`);

        // 3. Boş post kontrolü
        if (!isAnketMode && !content?.trim() && (!req.files || req.files.length === 0)) {
            console.error('❌ Boş içerik');
            return res.status(400).json({ error: 'İçerik veya medya eklemelisiniz', code: 'EMPTY_CONTENT' });
        }

        // 4. Kullanıcı kontrolü
        const user = await db.get('SELECT id, username, name, profilePic, isVerified, userType FROM users WHERE id = ?', req.user.id);
        if (!user) {
        ...
        console.log(`👤 Kullanıcı: ${user.username} (${user.id})`);
        ...
        if (req.files && req.files.length > 0) {
            console.log(`📁 ${req.files.length} dosya işlenecek...`);
            ...
            try {
                ...
                if (isVideo) {
                    // ✅ VİDEO İŞLEME - İYİLEŞTİRİLDİ
                    console.log(`🎬 Video işleniyor: ${file.originalname} (${(file.size / 1024 / 1024).toFixed(2)} MB)`);
                    ...
                    console.log(`📋 Video kopyalanıyor: ${file.path} -> ${outputPath}`);
                    ...
                    console.log(`✅ Video kaydedildi: ${filename} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
                    
                    media = `/uploads/videos/${filename}`;
                    finalMediaType = 'video';
                    
                    // Thumbnail oluştur (arka planda, beklemeden)
                    ...
                    createVideoThumbnail(outputPath, thumbPath)
                        .then(() => console.log(`✅ Thumbnail oluşturuldu: ${thumbFilename}`))
                        .catch(err => console.error(`⚠️ Thumbnail hatası: ${err.message}`));
                    
                } else {
                    // ✅ RESİM İŞLEME - İYİLEŞTİRİLDİ
                    console.log(`📷 Resim işleniyor: ${file.originalname} (${(file.size / 1024 / 1024).toFixed(2)} MB)`);
                    ...
                    try {
                        ...
                        console.log(`✅ Resim kaydedildi: ${filename} (${(stats.size / 1024).toFixed(2)} KB)`);
                        
                        media = `/uploads/posts/${filename}`;
                        finalMediaType = 'image';
                        
                    } catch (sharpError) {
                        console.error(`⚠️ Sharp hatası: ${sharpError.message}, direkt kopyalanıyor...`);
                        
                        // Sharp başarısız olursa orijinali kopyala
                        const ext = path.extname(file.originalname).toLowerCase();
                        ...
                        console.log(`✅ Resim direkt kopyalandı: ${fallbackFilename} (${(stats.size / 1024).toFixed(2)} KB)`);
            ...
            try {
                ...
                if (Array.isArray(opts) && opts.length > 0) {
                    pollOptionsJson = JSON.stringify(opts.map((opt, i) => ({ 
                        id: i, 
                        text: String(opt).substring(0, 200), 
                        votes: 0 
                    })));
                } else {
                    throw new Error('Anket seçenekleri geçersiz');
                }
            } catch (pollError) {
                console.error(`❌ Anket parse hatası: ${pollError.message}`);
                return res.status(400).json({ 
                    error: 'Anket seçenekleri hatalı', 
                    code: 'INVALID_POLL_OPTIONS' 
                });
            }
        }

        console.log(`💾 Veritabanına kaydediliyor: ${postId}`);
        console.log(`   - Tip: ${isAnketMode ? 'poll' : finalMediaType}`);
        console.log(`   - Medya: ${media || 'yok'}`);
        ...
        try {
            ...
            console.log(`✅ Veritabanına kaydedildi: ${postId}`);
        ...
        // 7. Post'u getir
        console.log(`📖 Post getiriliyor: ${postId}`);
        const post = await db.get(
            `SELECT 
                p.*, 
                u.profilePic as userProfilePic, 
        ...
        if (!post) {
            console.error(`❌ Post getirilemedi: ${postId}`);
            throw new Error('Post oluşturuldu ama getirilemedi');
        }

        // 8. Media URL'leri ekle
        if (post.media) {
            const filename = path.basename(post.media);
            if (post.mediaType === 'video') {
                post.mediaUrl = `/uploads/videos/${filename}`;
        ...
        // 9. Socket emit (real-time update)
        if (io) {
            io.emit('new_post', { 
                post: { 
                    ...post, 
            ...
            console.log(`📡 Socket emit yapıldı: new_post`);
        }
        ...
        console.log(`   - Tip: ${post.mediaType}`);
        console.log(`   - İçerik uzunluğu: ${postContent.length} karakter`);
        ...
        const processingTime = Date.now() - startTime;
        console.error(`❌ POST HATASI (${processingTime}ms):`, error.message);
        console.error(`   Stack:`, error.stack);
        
        // Tüm geçici dosyaları temizle
        console.log(`🧹 Hata sonrası dosya temizliği...`);
        if (req.files) {
            for (const file of req.files) {
                try {
                    await fs.unlink(file.path);
                    console.log(`   ✅ Temizlendi: ${file.path}`);
                } catch (unlinkErr) {
                    console.warn(`   ⚠️ Dosya silinemedi: ${file.path}`);
                }
            }
        }
        
        // Kullanıcı dostu hata mesajı
        let userMessage = 'Gönderi oluşturulamadı';
        let errorCode = 'INTERNAL_ERROR';
        
        if (error.message.includes('Video')) {
            userMessage = 'Video yüklenirken bir hata oluştu. Lütfen tekrar deneyin.';
            errorCode = 'VIDEO_PROCESSING_ERROR';
        } else if (error.message.includes('Resim') || error.message.includes('image')) {
            userMessage = 'Resim yüklenirken bir hata oluştu. Lütfen tekrar deneyin.';
            errorCode = 'IMAGE_PROCESSING_ERROR';
        } else if (error.message.includes('Veritabanı') || error.message.includes('database')) {
            userMessage = 'Veritabanı hatası. Lütfen birkaç saniye sonra tekrar deneyin.';
